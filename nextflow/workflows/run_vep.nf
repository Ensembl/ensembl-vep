...
include { chrosVEP } from '../nf_modules/run_vep_chros.nf'
 ...
 if (params.help) {
    ...
    log.info ''
    log.info 'Usage: '
    log.info '  nextflow -C nf_config/run_vep.config run workflows/run_vep.nf --vcf /mnt/d/work/repos/nf_genomics_pipelines/test-data/test.vcf.gz --chros 1,2  '
    log.info ''
    log.info 'Options:'
    log.info '  --vcf VCF	VCF that will be splited.'
    log.info '  --outdir DIRNAME          Name of output dir. Default: outdir'
    log.info '  --vep_config FILENAME     VEP config file'

...
sub annotate_InputBuffer {
  ...
  foreach my $tr(@{$self->get_all_features_by_InputBuffer($buffer)}) {
    ...
    foreach my $vf(@$vfs) {
      ...
      {
        $seq_mismatch = 1 unless $tr->{"slice"}->{"seq_region_name"} eq $vf->{"slice"}->{"seq_region_name"};
      }
...
sub filter_transcript {
  ...
  if(
    ...
    (
      ...
      ($self->{source_type} eq 'merged' && ($t->{_source_cache} || '') eq 'RefSeq')
    ) && (
      !(($t->stable_id || '') =~ /^[A-Z]{2}\_\d+/ ||
      (($t->seq_region_name || '') eq 'MT' && ($t->stable_id || '') =~ /^\d{4}$|^(rna-)?[A-Z0-9]{3,}$/) ||

...
my $runner = get_annotated_buffer_runner({
  ...
  tab => 1,
  quiet => 1,
  show_allele_ref => 1 # Include reference allele in output (and header)
});
is(
  $runner->get_OutputFactory->headers->[-2].$runner->get_OutputFactory->headers->[-1],
  "## test : header".
  "#Uploaded_variation\tLocation\tAllele\tGene\tFeature\tFeature_type\tConsequence\tcDNA_position\tCDS_position\tProtein_position\tAmino_acids\tCodons\tExisting_variation\tALLELE_REF\tIMPACT\tDISTANCE\tSTRAND\tFLAGS\ttest",
  'headers - plugin'
);
